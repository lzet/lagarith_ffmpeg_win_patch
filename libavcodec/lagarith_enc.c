...
static av_cold int lag_encode_init(AVCodecContext *avctx) {
    ...
    DWORD sz_bmih = sizeof (BITMAPINFOHEADER);
    ZeroMemory(&l->bmih, sz_bmih);
    ZeroMemory(&l->bmiho, sz_bmih);
	
	l->bmih.biSize = sz_bmih;
...
static int lag_encode_frame(AVCodecContext *avctx, AVPacket *avpkt, const AVFrame *frame, int *got_packet_ptr) {
	...
	ret = ICCompress(l->hic,frame->key_frame?ICCOMPRESS_KEYFRAME:0,
					&l->bmiho,l->odata,
					&l->bmih,l->cdata,
					NULL,&flags,frame->coded_picture_number,0,0,NULL,NULL);
	if(ret != ICERR_OK)
		return ret;
	
	ret = ff_alloc_packet2(avctx,avpkt,l->bmiho.biSizeImage, l->bmiho.biSizeImage);
	if(ret < 0)
		return ret;
